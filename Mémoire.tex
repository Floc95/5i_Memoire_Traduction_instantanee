...
\begin{document}
	...
	\section*{Résumé}
		...
		\paragraph{}Nous proposons ici une étude sur la traduction instantané voix vers voix dans le cadre d'une réunion. L'objectif principale sera de proposer une solution viable dans l’environnement d'une sale de réunion et à l'aide des technologies disponible. Nous aborderons aussi plus en détail la gestion des émotions dans le processus : nous pensons en effet que leur interprétation et restitution est un élément clef dans l'élaboration d'un système voix vers voix pour permettre une plus fine compréhension du locuteur dans le court laps de temps qui est imposé par la dynamique d'une conversation.
		\paragraph{}Nous traiterons donc dans un premier temps de l'ensemble des modules nécessaires à la traduction voix vers voix avant de nous attarder sur la prise en compte des émotions et les limites actuels.
	...
	\section*{Abstract}
		...
		\paragraph{}In this work we aim at exploring speech to speech translation in real time in the closed environment of a meeting room. The main goal is to propose a viable solution fitting a meeting room's conditions with today's technologies. Next we will adress how emotions can be handled by the system and how processing emotions can give an edge when trying to understanding what is being said in such a fast paced situation as a conversation.
	...
	\chapter{Introduction à la problématique}\label{introduction}
		...
		\section{Les composantes}\label{composantes}
			...
			\paragraph{}Premièrement, nous allons devoir déterminer le contexte de la future discutions pour notre système. En effet, que le système sache dans quelle contexte se situe la discussion est très important car il permettra d’améliorer grandement la fiabilité de la traduction. Lors de la traduction le niveau de langage approprié pourra être choisi, ce qui rendra la traduction plus fidèle.
			...
			\paragraph{}Pour ce qui est de la seconde question, c’est-à-dire de comment connaître la langue, il s’agira de la détecter en fonction de ce que la personne dira. Un peu à la manière d’un Google Translate avec l’écrit. Mais nous reviendront sur la réalisation de cette tâche plus tard.
	...
	\chapter{Résolution du système}\label{system}
		...
		\section{Reconnaissance automatique de la parole}\label{speechtotext}
			...
			\paragraph{}Cette technique est fréquemment utilisé de nos jours. Que ce soit dans les serveurs vocaux (messageries, assistance...), sur un ordinateur avec la dictée vocale ou dans les assistants personnels (Google Now, Siri).

			\subsection{Fonctionnement générale}

				\paragraph{Petit historique} 

				\paragraph{}Les premiers systèmes de RAP numériques datent d'il y'a plus de cinquante ans, ces premières solutions était limité à la reconnaissance de mots isolé dans un vocabulaire très limité (cardinal d'une dizaine de mots). Nous nous intéresserons ici uniquement aux techniques modernes permettant de couvrir une langue complète et de reconnaître de la parole continue, soit des phrases et discours complets ayant un sens.

				\paragraph{Parole continue}
				\paragraph{}Les système de reconnaissance de parole continue reçoivent en entrée un signal (analogique ou numérique) correspondant à un message orale à retranscrire en texte, ce qui pose un certain nombre de difficultés successive. La continuité de texte impose au système de :
				\begin{itemize}
				...
				\subsubsection{Étude du signal (décodage acoustico-phonétique)}
					...
					\paragraph{}L'étude du signal s'opère le plus le plus souvent via une analyse paramétrique de ce dernier et permets ainsi de retrouver l'élément acoustique le plus probable correspondant au signale.
					...
					\paragraph{}Pour ce faire il existe plusieurs modèles notamment les modèles Markoviens Cachées, les modèles neuronimétriques ou encore les algorithme de comparaison dynamique.
				...
				\subsection{Retranscription des émotions}
					...
					\subsubsection{Retranscription par synthèse vocale}
						...
						\paragraph{}Ici la synthèse se fait via la concaténation d'une série de modèles de Markov caché dépendant du contexte qui détermineront la durée et les fondamentales du spectre puis la forme de l'onde est ensuite généré via un filtre dit de Mel Log Spectrum Approximation.
							\paragraph{}Ce système présente l'avantage d'être capable d'émuler plus finement des émotions en jouant sur des nuances. Cette approche présente l'avantage de nécessiter une base de donnée bien plus réduite.						
	...
	\chapter{Aller plus loin}\label{system}
		...
		\paragraph{}Nous l'avons vu, si les techniques actuels de synthèse vocale sont maintenant capable de reproduire une voix presque naturel lors de l'émulation d'une voix neutre il en est tout autrement lors ce que l'on tente d'incorporer des émotions dans le signal. Si la technique actuel ne permet pas encore aux modèles de Markov caché de rivaliser avec les systèmes à sélection d'unité les plus perfectionné ils sont bien l'avenir de la synthèse vocale. En effet, ces dernier on l'avantage de pouvoir donner des résultat correcte à partir de corpus modeste et sont finalement plus performant lors ce qu'il s'agit produire une voix neutre notamment grâce à leur capacité à suivre une prosodie plus naturel. De plus, le plus gros point faible des système à modèle de Markov caché réside dans la qualité des voix généré qui sont dégradé, néanmoins de nouvelles solutions alternative à celle exposé ici sont actuellement en développement, certaine tentant de reproduire le système de modulation de la voix chez l'humain.
		\paragraph{}Nous sommes tout de même encore loin de reproduire le langage naturel via un synthétiseur vocale, mais les avancés qui on été effectué sur les voix neutres permettant aujourd'hui de produire des voix satisfaisante à l'écoute, l'étape suivante tend invariablement à l'amélioration de la simulation des émotions.
