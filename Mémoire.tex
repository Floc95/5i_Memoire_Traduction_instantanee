...
\begin{document}
	...
	\section*{Résumé}
		...
		\paragraph{}Nous proposons ici une étude sur la traduction instantanée voix vers voix dans le cadre d'une réunion. L'objectif principale sera de proposer une solution viable dans l’environnement d'une salle de réunion et à l'aide des technologies disponibles. Nous aborderons aussi plus en détail la gestion des émotions dans le processus : nous pensons en effet que leur interprétation et restitution est un élément clef dans l'élaboration d'un système voix vers voix pour permettre une plus fine compréhension du locuteur dans le court laps de temps qui est imposé par la dynamique d'une conversation.
		\paragraph{}Nous traiterons donc dans un premier temps de l'ensemble des modules nécessaires à la traduction voix vers voix avant de nous attarder sur la prise en compte des émotions et des limites actuels.
	...
	\chapter{Introduction à la problématique}\label{introduction}
		...
		Avant toute chose, nous allons commencer par définir le scénario, sans quoi, il serait difficile de se poser des limites dans nos réflexions.
			...
			\paragraph{}Dans un cadre professionnel, dix personnes de nationalités différentes et donc ne parlant pas la même langue se joignent autour d’une table afin d’avoir une réunion. Chaque personne parlera dans sa langue maternelle.
			\paragraph{}Afin que tout le monde puissent comprendre ce que les autres disent et que la réunion soit dynamique, un système sera mis en place afin de traduire les paroles de chaque personne et de retranscrire vocalement la traduction. Ce système sera relativement poussé puisqu’il traduira en temps réel, c’est-à-dire qu’il fera la traduction alors même que l’interlocuteur n’a pas fini sa phrase. De plus, ce système sera capable d’analyser les émotions dans la voix et de la restituer dans la synthèse vocale.
			\paragraph{}Nous allons devoir identifier chaque composante de ce système dans notre scénario et affiner celui-ci.

		\section{Les composantes}\label{composantes}

			\paragraph{Le contexte}

			\paragraph{}Premièrement, nous allons devoir déterminer le contexte de la future discutions pour notre système. En effet, que le système sache dans quel contexte se situe la discussion est très important car il permettra d’améliorer grandement la fiabilité de la traduction. Lors de la traduction le niveau de langage approprié pourra être choisi, ce qui rendra la traduction plus fidèle.

			\paragraph{}On pourrait imaginer que le système analyse la discussion afin de déterminer ce contexte mais le système serait moins efficace le temps de cette analyse. C’est pourquoi nous allons partir du principe que quelqu’un initialise le contexte avant la réunion (via une interface par exemple). Le contexte choisi par cette personne sera donc « Professionnel ».

			\paragraph{Identification et langues}

			\paragraph{}Une fois le contexte choisi, la réunion peut commencer. Les personnes commenceront alors à parler. Ici nous trouvons alors un autre problème : Comment détecter qui parle ? Et dans quelle langue ?

			\paragraph{}Afin de répondre à la première question, nous avons pensé à un système qui analyse les fréquences des voix ainsi que leurs signatures afin d’identifier chaque personne si le système n’avait qu’un micro central. Cela fonctionnerait certainement mais dans ce cas, comment restituer dans la langue de la personne la traduction sans avoir une cacophonie de traduction dans la salle ? Il nous paraît alors évident qu’il soit nécessaire que chaque participant ai une oreillette dans laquelle on lui traduit les discussions.

			\paragraph{}Afin d’éviter un processus d’identification des propriétaires par rapport à leur voix afin de ne pas leur traduire leur propre parole, il nous a paru plus simple de supposer que les oreillettes contiennent un micro dans lequel parle l’interlocuteur. De cette manière, chaque personne sera identifiée de manière simple et sûre.

			\paragraph{}Pour ce qui est de la seconde question, c’est-à-dire de comment connaître la langue, il s’agira de la détecter en fonction de ce que la personne parlera. Un peu à la manière d’un Google Translate avec l’écrit. Mais nous reviendront sur la réalisation de cette tâche plus tard.

			\paragraph{Reconnaissance automatique de la parole}

			\paragraph{}A ce stade, il nous faut encore réussir à mettre sous format textuel ce qu’une personne dit afin de le traduire par la suite. Il existe des logiciels spécialisé dans ce travail, on les appelle des logiciels de Speech To Text, ou en français : \og Reconnaissance automatique de la parole \fg. Il existe une multitude de logiciels comme ceci, c’est pourquoi nous n’allons pas plus en parler pour le moment et estimer que ce travail est fait.

			\paragraph{Traduction}

			\paragraph{}Maintenant que l’on dispose du texte, nous pouvons enfin le traduire dans toutes les autres langues présentes dans la salle. Pour ce faire nous allons devoir utiliser un traducteur, seulement, pour parfaire la traduction, le traducteur devra prendre en compte le contexte de la discussion afin de choisir les traductions les plus adaptées. 

			\paragraph{Synthèse vocale}

			\paragraph{}Il est désormais temps de restituer la traduction à chaque interlocuteur. Pour réaliser cela, nous allons devoir faire appel à un système de synthèse vocale. Une synthèse vocale a pour but d’exprimer un texte par la parole, de la manière la plus naturelle et humaine possible dans notre cas. De nos jours, il existe une multitude de logiciels de synthèse vocale, nous en parlerons plus en détails dans une autre partie.
			...
			\paragraph{}La composantes de l'émotion est très intéressante car encore absente des solutions temps réel actuel, sa détection et synthèse en temps réel est un problème toujours non résolu alors que son apport en terme de dynamisme comme d'aide à la compréhension -qui sont deux facteurs très important pour un système temps réel- sont très importants. C'est pourquoi nous nous pencherons plus particulièrement sur ce facteur et nous détaillerons comment la détecter, comment la comprendre et comment la retransmettre dans les limites imposés de notre scénario.
			
			\paragraph{}Dans un premier temps nous aborderons les solutions existantes permettant de répondre à chacune des étapes de notre problématique avant de faire un point plus détailler sur la reconnaissance et la retranscription des émotions. Pour finir nous évoquerons les limitations inhérentes à notre solution.
	...
	\chapter{Résolution du système}\label{system}
		...
		\section{Reconnaissance automatique de la parole}\label{speechtotext}
			...
			\subsection{Fonctionnement générale}
				...
				\paragraph{}Les premiers systèmes de RAP numériques datent d'il y'a plus de cinquante ans, ces premières solutions était limité à la reconnaissance de mots isolés dans un vocabulaire très limité (cardinal d'une dizaine de mots). Nous nous intéresserons ici uniquement aux techniques modernes permettant de couvrir une langue complète et de reconnaître de la parole continue, soit des phrases et discours complets ayant un sens.

				\paragraph{Parole continue}
				\paragraph{}Les systèmes de reconnaissance de parole continue reçoivent en entrée un signal (analogique ou numérique) correspondant à un message orale à retranscrire en texte, ce qui pose un certain nombre de difficultées successives. La continuité de texte impose au système de :
				\begin{itemize}
					...
					\item de vérifier que le résultat obtenue est sémantiquement correcte, chaque phrase doit avoir un sens
				\end{itemize}
				...
				\paragraph{}Tout d'abord le décodage phonétique s'opère en analysant par fenêtre le signal d'origine puis en comparant ses composés à la bibliothèque d'unités connue utilisée par le système (mot, syllabe, diphone, phonème, etc.).
				Dans un second temps les informations décodées doivent être traitées pour obtenir des phrases syntaxiquement et sémantiquement correcte, pour ce faire les systèmes actuels reposent sur des procédés statistiques étudiant la probabilité d'une suite de mots et ajustant ainsi les séquences trouvé.

				\subsubsection{Étude du signal (décodage acoustico-phonétique)}

					\paragraph{}L'étude du signal s'opère le plus souvent via une analyse paramétrique de ce dernier et permets ainsi de retrouver l'élément acoustique le plus probable correspondant au signal.
					...
					\paragraph{}Malheureusement il n'est pas possible d'effectuer directement une tel opération, la durée de chaque unité à reconnaître n'étant pas stable il faut pouvoir faire coïncider le signal relevé avec les données de notre bibliothèque via des procédés de normalisation temporel.
					\paragraph{}Pour ce faire il existe plusieurs modèles notamment les modèles Markoviens Cachées, les modèles neuronimétriques ou encore les algorithmes de comparaison dynamique.

					\paragraph{}La technique la plus couramment utilisé aujourd'hui se fait par modèles de Markov caché. Il est alors possible de retrouver l'ensemble des états probablement parcouru à partir des observations faites en isolent le chemin le plus probable.							

				\clearpage

				\subsubsection{Reconstruction sémantique}

					\paragraph{}Une fois les unités phonétiques de bases isolées il reste alors à identifier les formes lexicales correspondantes. Cette étape est appelé le décodage. Les techniques de décodages reposent sur des graphes on retrouve alors des résolutions se basant sur l'algorithme A*. On génère alors un graphe où les nœuds comportent tous les mots probables composés par les unités phonétique préalablement isolés.
			...
			\subsection{Et pour nous ?}
				...
				\paragraph{}Pour ce qui est du problème de parole concurrente (deux interlocuteurs se mettent à parler en même temps), celui-ci pose deux problèmes : séparer ce que disent les deux intervenants pour permettre d'identifier ce que chacun dit, puis retranscrire le résultat à l'auditoire. S'il est certainement possible de séparer ce que dit chaque intervenant cela demanderait des traitement supplémentaire qui ne sont d'après nous pas justifiable : comment retranscrire ensuite à l'auditoire ce qui est dit ? Le résultat de chaque traduction ne peut pas être rendu en même temps, aucun ne serait alors intelligible et si l'on décale la synthèse de l'une des traduction quand la passer ? Elle se retrouverait déplacé à plus tard indéfiniment à mesure que d'autres intervenants prendraient la parole. Nous avons donc trouvé préférable d'ignorer ce cas en nous reposant sur la discipline des participants.

				\paragraph{}Quand aux techniques de reconnaissances se posent trois questions : l'unité à utiliser, le modèle de décodage acoustico-phonétique et l'algorithme de reconnaissance lexical/grammatical; ce dernier étant directement liée au choix du modèle de décodage acoustico-phonétique. 
				...
				\paragraph{}Les autres paramètres n’ayant pas d’impact particulière sur le cœur de notre recherche nous sélectionnerons le système le plus utilisé, les Modèles de Markov cachés, celui-ci ayant déjà prouvé son efficacité.
		...
		\section{Traduction}\label{traduction}
			...
			\subsection{Solutions}
				...
				\subsubsection{Traduction par l'exemple}
					...
					Cependant, cette traduction nécessite une base de données très fournie pour fonctionner correctement. Cette base de données devra être alimentée par des textes de références traduit dans chaque langue. Il s'agit d'un lourd investissement.

			\clearpage

			\subsection{Notre choix}

				\paragraph{}Après avoir analyser les différents types de traductions automatiques, nous pouvons maintenant décider de quelle méthode de traduction nous allons nous servir. Dans le cadre de notre scénario, on souhaite ce qu'il y a de mieux, c'est pourquoi nous allons nous orienter vers le choix qu'on fait les leaders de ce marché : utiliser deux de ces règles. Nous allons donc utiliser à la fois la traduction par règle et la traduction statistique afin d'avoir un résultat le plus fiable possible.

				\paragraph{}En effet, les deux méthodes se complètent, la traduction automatique par règle est très bonne au niveau de la bonne formation des phrases, des verbes et autres, et la traduction automatique statistique est quant à elle performante pour des domaines de traductions spécifiques (le contexte, en sorte) et sa traduction d'expression ou mot à mot est plutôt performante également. De plus, ces deux méthodes nécessitent un corpus de texte bilingue volumineux, on pourra alors mutualiser ce corpus.
			...
			\section{Synthèse vocale}\label{synthese}
				...
				\begin{itemize}
					...
					\paragraph{}L'ensemble des modules permettant la décomposition du texte est désigné comme le module de traitement automatique du langage naturel. Plusieurs traitements consécutif sont alors exécuté sur le texte pour le transcrire sous une forme lisible par le synthétiseur vocale. On retrouve alors les traitements principaux suivant : 
					
					\paragraph{La phase de nettoyage}
					\paragraph{}Le but est d’aplanir le texte en y remplaçant toutes les occurrences d'abréviations, de chiffre et nombre, de caractère spéciaux, unités de mesures ou encore devises par leur équivalent en toutes lettres transformant alors « num. tel. 01.00.00.00.01 \$2 la minute à partir de 08h30 »  en « numéro téléphone  zéro un, zéro zéro, zéro zéro, zéro zéro, zéro un deux dollars la minute à partir de huit heure trente ».
					\paragraph{}Dans les langues concernés, une étape de ré-accentuation peut être à prévoir par exemple dans les cas où les accents ne seraient pas présent de mots en majuscule. Lors de cette étape, il peut aussi être intéressant de repérer et traiter les acronymes comportant des voyelles qui ne sont pas lu comme un mot, et ainsi décomposer l'acronyme « PIB » est nécessaire alors que « OTAN » ou encore « PHP » peuvent être laisser comme tel.
			...
			\section{Émotions}\label{emotions}
				...
				\subsection{Identification des émotions}
					...
					\subsubsection{Quels sont les facteurs d'identification des émotions ?}
						...
						L'identification des émotions, de manière générale, est identifiable par 4 facteurs :
						\begin{itemize}
						...
						\paragraph{}Dans le cas où il s’agirait d'un tique, la personne va par exemple se ronger les ongles si elle commence a être stressé ou inquiète. Mais dans le même cas, une autre personne pourrait commencer à « jouer » avec ses doigts, les taper sur une table ou autre. En fait, ce genre de réaction face à une émotion dépend de chaque personne et peut varier selon les gens. Cependant, on peut quand même identifier certaines réaction, qui sont plutôt universelle dans le sens de ce qu'elle traduise (se ronger les ongles par exemple, tous le monde ne le fait pas mais lorsque quelqu'un le fait, c'est généralement parce qu'il est stressé).
						\paragraph{}En outre, dans le cas où il s'agirait d'une réaction provoqué par une émotion forte, cela se traduit généralement plus à un changement de comportement physique. Par exemple, en cas de fort énervement, il arrive qu'une personne commence à « gesticuler », qu'elle ne tienne plus en place. Ses mouvements deviennent plus rapides et plus fréquents. Ce genre de réactions sont plus généralisés chez les individus que la présence de tique, néanmoins toute personne est différente et il peut arriver qu'une personne ne réagisse pas de la même façon face à une émotion forte.
						
						\paragraph{L'activité physique du visage}
						\paragraph{}Le visage d'un individu est l'élément physique qui transmet le plus d'émotions. En effet, le visage est la partie du corps qui exprime le plus les sentiments (et donc les émotions) d'un individu. De nombreuses études ont été menés afin d'identifier clairement les réactions faciales des individus face à une émotion. Nous connaissons maintenant chaque interprétations possible d'une émotion chez un humain.
					...
					\subsubsection{Comment allons-nous les identifier ?}
						...
						\paragraph{}Parallèlement, nous analyserons les marques sémantiques de la phrase à traduire. Si les marques sémantiques peuvent desceller une émotion dans la phrase, on calculera la probabilité de l'exactitude que ce soit cette émotion là. Nous aurons déjà une probabilité pour l'émotion déterminée avec les données phonologiques, nous choisirons alors la probabilité la plus forte entre les deux système. Les probabilités pourront ne pas être équivalente, il pourrait arriver qu'une probabilité de 80\% en données phonologiques soit supérieure en chance de réussite à une probabilité de 90\% en marques sémantiques. C'est pourquoi, il faudra entraîner notre système avec des humains qui vérifierons les résultats fournis par le système et qui ajusterons le calcul de la détermination de l'émotion choisi. Cette phase d’entraînement du système est très importante afin d'avoir un système juste et fiable.
				...
				\subsection{Retranscription des émotions}
					...
					\subsubsection{Retranscription par synthèse vocale}
						...
						\paragraph{}Ici la synthèse se fait via la concaténation d'une série de modèles de Markov caché dépendant du contexte qui détermineront la durée et les fondamentales du spectre puis la forme de l'onde est ensuite généré via un filtre dit de Mel Log Spectrum Approximation.
	...
	\chapter{Aller plus loin}\label{system}
		...
		\paragraph{}Un deuxième axe d'amélioration se situe au niveau de la traduction, et plus particulièrement de la prise en compte des données émotionnelle dans celle-ci. Il serait ainsi intéressant de se pencher sur la sélection du vocabulaire (niveau, nuance) à employer lors de la traduction en incorporant dans les paramètres de cette dernière l'émotion détecté lors de la prononciation de la phrase. Cela permettrait d'appuyer sur l'état d'esprit du locuteur de façon naturel via l'intégration d'un lexique contextualisé.
		
		\subsection{Synthèse de l'émotion}
		
		\paragraph{}Nous l'avons vu, si les techniques actuelles de synthèse vocale sont maintenant capable de reproduire une voix presque naturelle lors de l'émulation d'une voix neutre il en est tout autrement lors ce que l'on tente d'incorporer des émotions dans le signal. Si la technique actuelle ne permet pas encore aux modèles de Markov caché de rivaliser avec les systèmes à sélection d'unité les plus perfectionné ils sont bien l'avenir de la synthèse vocale. En effet, ces dernier on l'avantage de pouvoir donner des résultats corrects à partir de corpus modeste et sont finalement plus performant lors ce qu'il s'agit produire une voix neutre notamment grâce à leur capacité à suivre une prosodie plus naturelle. De plus, le plus gros point faible des système à modèle de Markov caché réside dans la qualité des voix généré qui sont dégradé, néanmoins de nouvelles solutions alternative à celle exposée ici sont actuellement en développement, certaine tentant de reproduire le système de modulation de la voix chez l'humain.
		\paragraph{}Nous sommes tout de même encore loin de reproduire le langage naturel via un synthétiseur vocale, mais les avancées qui on été effectuées sur les voix neutres permettent aujourd'hui de produire des voix satisfaisante à l'écoute, l'étape suivante tend invariablement à l'amélioration de la simulation des émotions.
