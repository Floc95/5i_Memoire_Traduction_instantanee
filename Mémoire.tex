...
\begin{document}
	...
	\chapter{Introduction à la problématique}\label{introduction}
		...
		\section{Les composantes}\label{composantes}
			...
			\paragraph{}Premièrement, il faut déterminer le contexte de la future discussion à traduire pour notre système. En effet, la détermination du contexte de la discussion est primordiale puisqu'elle permet d'améliorer grandement la fiabilité du système et la pertinence\textbf{} de la traduction.Lors de la traduction le niveau de langage approprié peut être choisi, ce qui rendra la traduction plus fidèle au propos initial.
	...
	\chapter{Résolution du système}\label{system}
		...
		\section{Reconnaissance automatique de la parole}\label{speechtotext}
			...
			\subsection{Et pour nous ?}
				...
				\paragraph{}Les autres paramètres n’ayant pas d’impact particulier sur le coeur de notre recherche nous sélectionnerons le système le plus utilisé, les Modèles de Markov cachés, celui-ci ayant déjà prouvé son efficacité.
...
%%NEW STUFF BEGIN					
					\paragraph{}Le challenge dans notre scénario au niveau de la synthèse vocale réside dans la retranscription des émotions. Avant de nous pencher sur la sélection d'un système de synthèse vocale, nous aborderons trois points :  
					\begin{itemize}
							\item \textbf{}Les participants sont hautement susceptibles d'appartenir à des culture différentes 
							\item \textbf{}Notre système ne comprend qu'une synthèse vocale 
							\item \textbf{}Le cadre de notre scénario mène à une faible présence d'émotions poussée à l'extrême 
					\end{itemize}
Notre système devra pouvoir restituer des émotions dans plusieurs langues ce qui implique très probablement que les personnes présentent ne seront pas du même milieu culturel, il est alors préférable que la méthode choisi soit capable de s'adapter à de potentiels différences dans la vocalisation des émotions selon les cultures.  
%%TODO add sources

				\paragraph{}Il semblerait heureusement que la reconnaissance des émotions dans un contenu vocalisé soit très majoritairement indépendante des milieux culturels du locuteur et de l'interlocuteur. Il ne nous est alors pas nécessaire de s'inquiéter de l'adaptabilité du système choisi à toutes les langues. 

				\paragraph{}Notre dispositif ne comprend pas non plus de reproduction de visage qui pourrait aider à exprimer une émotions, mais dans notre contexte, les participants sont présents dans une même pièce. Les expressions du corps et du visage sont alors visible directement sur le locuteur. De ce fait, un autre risque peut immerger : notre système ayant forcément un délai entre la locution originel et la retranscription, en cas de changement rapide dans les émotions véhiculées par l'orateur, les indices visuels pourrait ne pas corrélé avec ce que les interlocuteurs sont entrain d'entendre. Ne voyant pas de solutions commodes nous avons choisi d'ignorer ce problème. 

				\paragraph{}Dans une réunion classique, le ratio entre phrases légèrement tintées d'émotions et marques d'émotions extrême tourne fortement en faveur de la nuance. Notre choix devra donc se porter sur un système compétent dans la synthèse de multiple nuance d'une émotion. Ce choix est d'autant plus appuyé par notre contexte, si nous considérons que tous les participants sont dans une même pièce en cas d'éclat vif d'une émotion cela sera perceptible par tous, directement.
...
%% NEW STUFF END					
						...
						\paragrap{}Si les systèmes à sélection d'unité son aujourd'hui les plus performants pour reproduire un éventail près-délimité d'émotions, et ce avec un résultat naturel, ils sont aussi incapables de nuance : il faudrait enregistrer chaque nuance au préalable, ce qui n'est pas une solution praticable. Plus récemment un nouveau système, bien que moins efficace pour la restitution d'une émotion appuyée, capable de nuances, et demandant un travail d'enregistrement moindre a été développé.
						...
						\paragraph{}Ici la synthèse se fait via la concaténation d'une série de modèles de Markov caché dépendant du contexte qui détermineront la durée et les fondamentales du spectre, puis la forme de l'onde est générée via un filtre dit de "`Mel Log Spectrum Approximation"'.
							\paragraph{}Ce système présente l'avantage d'être capable d'émuler plus finement des émotions en jouant sur des nuances, de plus en modulant le signal à posteriori il n'est plus nécessaire d'enregistrer la totalité des nuances désiré à l'avance.
					...
					\paragraph{}Tout fois, quelque soit le système employé, les différents tests ont montré que les systèmes de synthèse vocales ne permettent pas à un écouteur de discerner plusieurs émotions aux signes vocaux proches (joie/surprise). De plus, certaines émotions telles que le dégoût on été très mal reconnues par les écouteurs humains. Ces deux systèmes sont donc encore sujets à des améliorations.
%% NEW STUFF					
					\subsubsection{Pour nos besoins}

					\paragraph{}Les applications les plus répandues pour de la voix de synthèse qui requière d'être capable de simuler une émotion demandent un échantillon étroit d'émotions, comme par exemple un service téléphonique automatisé pour lequel il est possible de déterminer à l'avance qu'elle type d'intonation est désirable : pour annoncer qu'un participant a gagné à un concourt une voix enjoué sera sélectionné alors qu'un ton plus triste sera prit pour annoncer les perdants. Les deux cas sont connues à priori et peuvent être fixés.
					
					\paragraph{}Dans notre cas nous n'avons pas la possibilité de connaître à l'avance le type d'émotions à simuler ni leurs degrés et devons alors choisir un système apte à reproduire un large panel d'émotions. De plus, notre scénario nous impose d'être à même de synthétiser un nombre de langues suffisamment large pour pouvoir soutenir la traduction automatisé d'une rencontre international, ce qui multiplie la taille de la base de donnée par autant de langues à supporter. 
					
					\paragraph{}Il est aussi important de noter qu'un des freins à l'adoption d'un système modifiant le signal réside dans son naturel apparent moindre, ce qui n'a peut-être pas lieu d'être : le principe de l'\enquote{uncanny valley} (ou vallée dérangeante) dit que jusqu'à un certain point de raffinement dans la simulation de l'humain, plus on se rapproche du réel plus la moindre déformation ressort comme étant monstrueuse. Les synthèses vocales n'étant toujours pas parfaites le naturel perdu dans l'utilisation de synthèse touchant au signal sonore est donc rattrapé par la rigidité des systèmes ne reposant qu'uniquement sur le choix d'unités pouvant mené plus facilement à un décalage entre les propos et le ton.
					
					\paragraph{}Ces spécifications nous poussent vers le choix du deuxième système présenté ici : il est à la fois plus souple et sera alors capable de simuler toute une variété d'émotions à différents degrés tout en nécessitant un corpus de base plus réduit.
